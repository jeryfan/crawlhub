...
class DatabaseConfig(BaseSettings):
    ...
    def SQLALCHEMY_DATABASE_URI_SCHEME(self) -> str:
        return "postgresql+asyncpg" if self.DB_TYPE == "postgresql" else "mysql+aiomysql"

    @computed_field
    @property
    def SQLALCHEMY_DATABASE_URI_SCHEME_SYNC(self) -> str:
        return "postgresql+psycopg2" if self.DB_TYPE == "postgresql" else "mysql+pymysql"
...
class CeleryConfig(DatabaseConfig):
    ...
    def CELERY_RESULT_BACKEND(self) -> str | None:
        if self.CELERY_BACKEND in ("database", "rabbitmq"):
            ...
            db_extras = f"?{db_extras}" if db_extras else ""
            return (
                f"db+{self.SQLALCHEMY_DATABASE_URI_SCHEME_SYNC}://"
                f"{quote_plus(self.DB_USERNAME)}:{quote_plus(self.DB_PASSWORD)}@{self.DB_HOST}:{self.DB_PORT}/{self.DB_DATABASE}"
                f"{db_extras}"
            )
