...
def send_email_register_mail_task(language: str, to: str, code: str) -> None:
    ...
    try:
        ...
        logger.info(
            click.style(
                f"Send email register mail to {to} succeeded: latency: {end_at - start_at}",
                fg="green",
            )
        )
    except Exception:
...
@shared_task
def send_email_register_mail_task_when_account_exist(
    language: str, to: str, account_name: str
) -> None:
    """
    ...
    try:
        login_url = f"{app_config.CONSOLE_WEB_URL}/signin"
        reset_password_url = f"{app_config.CONSOLE_WEB_URL}/reset-password"
