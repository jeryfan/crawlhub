...
class OAuthServerService(BaseService):
    async def get_oauth_provider_app(self, client_id: str) -> OAuthProviderApp | None:
    ...
    def sign_oauth_access_token(
        ...
        match grant_type:
            case OAuthGrantType.AUTHORIZATION_CODE:
                redis_key = OAUTH_AUTHORIZATION_CODE_REDIS_KEY.format(
                ...
                access_token = OAuthServerService._sign_oauth_access_token(
                    client_id, user_account_id
                )
                refresh_token = OAuthServerService._sign_oauth_refresh_token(
                    client_id, user_account_id
                )
                return access_token, refresh_token
            case OAuthGrantType.REFRESH_TOKEN:
                redis_key = OAUTH_REFRESH_TOKEN_REDIS_KEY.format(
    ...
    async def validate_oauth_access_token(self, client_id: str, token: str) -> Account | None:
        redis_key = OAUTH_ACCESS_TOKEN_REDIS_KEY.format(client_id=client_id, token=token)
        user_account_id = redis_client.get(redis_key)
        if not user_account_id:
            return None
        ...
        return await AccountService(self.db).load_user(user_id_str)
