...
in FastAPI Template. It follows Domain-Driven Design principles with proper type hints and
...
class EmailI18nConfig(BaseModel):
    ...
    templates: dict[EmailType, dict[EmailLanguage, EmailTemplate]] = Field(
        ...
        description="Mapping of email types to language-specific templates",
    )
...
class EmailI18nService:
    ...
    def _render_email_content(
        ...
        template_path = (
            template_config.branded_template_path
            if branding.enabled
            else template_config.template_path
        )
        ...
        full_context = {
            **template_context,
            ...
            "application_title": (
                branding.application_title if branding.enabled else "FastAPI Template"
            ),
        }
...
def create_default_email_config() -> EmailI18nConfig:
    ...
    templates: dict[EmailType, dict[EmailLanguage, EmailTemplate]] = {
        ...
        EmailType.ACCOUNT_DELETION_VERIFICATION: {
            EmailLanguage.EN_US: EmailTemplate(
                subject="FastAPI Template Account Deletion and Verification",
            ...
            EmailLanguage.ZH_HANS: EmailTemplate(
                subject="FastAPI Template 账户删除和验证",
