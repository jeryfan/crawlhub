...
async def generate_key_pair(tenant_id: str) -> str:
    ...
    await storage.save(filepath, pem_private)
...
def decrypt(encrypted_text: bytes, tenant_id: str) -> str:
    ...
    return decrypt_token_with_decoding(
        encrypted_text=encrypted_text, rsa_key=rsa_key, cipher_rsa=cipher_rsa
    )
