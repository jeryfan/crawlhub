...
class KeywordsModeration(Moderation):
    ...
    async def moderation_for_inputs(self, inputs: dict, query: str = "") -> ModerationInputsResult:
        ...
        if self.config["inputs_config"]["enabled"]:
            ...
            flagged = await self._is_violated(inputs, keywords_list)
    ...
    async def moderation_for_outputs(self, text: str) -> ModerationOutputsResult:
        ...
        if self.config["outputs_config"]["enabled"]:
            ...
            flagged = await self._is_violated({"text": text}, keywords_list)
    ...
    async def _is_violated(self, inputs: dict, keywords_list: list) -> bool:
