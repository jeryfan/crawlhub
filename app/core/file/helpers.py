...
def get_signed_file_url(upload_file_id: str, as_attachment=False) -> str:
    url = f"{app_config.FILES_URL}/files/{upload_file_id}/file-preview"
...
def get_signed_file_url_for_plugin(
    filename: str, mimetype: str, tenant_id: str, user_id: str
) -> str:
    # Plugin access should use internal URL for Docker network communication
    base_url = app_config.INTERNAL_FILES_URL or app_config.FILES_URL
...
def verify_plugin_file_signature(
    *,
    ...
    secret_key = app_config.SECRET_KEY.encode()
    ...
    return current_time - int(timestamp) <= app_config.FILES_ACCESS_TIMEOUT
...
def get_public_file_url(upload_file_id: str) -> str:
    """获取公开访问的文件 URL（仅用于平台资源如 Logo、Favicon）"""
    return f"{app_config.FILES_URL}/files/{upload_file_id}/public"
