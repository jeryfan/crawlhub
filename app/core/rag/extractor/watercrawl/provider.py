...
class WaterCrawlProvider:
    ...
    def crawl_url(self, url, options: dict | Any | None = None):
        ...
        if options.get("crawl_sub_pages", True):
            ...
            spider_options["include_paths"] = (
                options.get("includes", "").split(",") if options.get("includes") else []
            )
            spider_options["exclude_paths"] = (
                options.get("excludes", "").split(",") if options.get("excludes") else []
            )

        wait_time = options.get("wait_time", 1000)
        page_options = {
            "exclude_tags": options.get("exclude_tags", "").split(",")
            if options.get("exclude_tags")
            else [],
            "include_tags": options.get("include_tags", "").split(",")
    ...
    def _get_results(
        ...
        while True:
            page += 1
            response = self.client.get_crawl_request_results(
                crawl_request_id, page, page_size, query_params
            )
