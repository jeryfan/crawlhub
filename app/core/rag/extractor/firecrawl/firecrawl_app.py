...
class FirecrawlApp:
    ...
    def map(self, url: str, params: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        headers = self._prepare_headers()
        json_data: dict[str, Any] = {"url": url, "integration": "fastapi"}
        if params:
    ...
    async def check_crawl_status(self, job_id) -> dict[str, Any]:
        headers = self._prepare_headers()
            ...
            if crawl_status_response.get("status") == "completed":
                ...
                if url_data_list:
                    ...
                    try:
                        ...
                        await storage.save(file_key, json.dumps(url_data_list).encode("utf-8"))
    ...
    def search(self, query: str, params: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        if response.status_code == 200:
            ...
            if not response_data.get("success"):
                raise Exception(
                    f"Search failed. Error: {response_data.get('warning', 'Unknown error')}"
                )
