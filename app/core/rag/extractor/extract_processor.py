...
class ExtractProcessor:
    @classmethod
    async def load_from_upload_file(
        cls,
        db: AsyncSession,
        ...
        if return_text:
            ...
            return delimiter.join(
                [
                    ...
                    for document in await cls.extract(db, extract_setting, is_automatic)
                ]
            )
        else:
            return await cls.extract(db, extract_setting, is_automatic)

    @classmethod
    async def load_from_url(
        cls, db, url: str, return_text: bool = False
    ) -> Union[list[Document], str]:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            extract_setting = ExtractSetting(
                datasource_type=DatasourceType.FILE, document_model="text_model"
            )
            if return_text:
                delimiter = "\n"
                return delimiter.join(
                    [
                        document.page_content
                        for document in await cls.extract(
                            db, extract_setting=extract_setting, file_path=file_path
                        )
                    ]
                )
            else:
                return await cls.extract(db, extract_setting=extract_setting, file_path=file_path)
        ...
        if extract_setting.datasource_type == DatasourceType.FILE:
            with tempfile.TemporaryDirectory() as temp_dir:
                if not file_path:
                    ...
                    await storage.download(upload_file.key, file_path)
                input_file = Path(file_path)
                file_extension = input_file.suffix.lower()
                etl_type = app_config.ETL_TYPE
                extractor: BaseExtractor | None = None
                if etl_type == "Unstructured":
                    unstructured_api_url = app_config.UNSTRUCTURED_API_URL or ""
                    unstructured_api_key = app_config.UNSTRUCTURED_API_KEY or ""
                    ...
                    if file_extension in {".xlsx", ".xls"}:
                        ...
                        extractor = WordExtractor(
                            db, file_path, upload_file.tenant_id, upload_file.created_by
                        )
