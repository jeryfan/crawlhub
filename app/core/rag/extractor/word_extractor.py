...
class WordExtractor(BaseExtractor):
    ...
    def __init__(self, db: AsyncSession, file_path: str, tenant_id: str, user_id: str):
    ...
    async def extract(self) -> list[Document]:
        ...
        content = await self.parse_docx(self.file_path)
        return [
            Document(
    ...
    async def _extract_images_from_docx(self, doc):
        image_count = 0
        image_map = {}
            ...
            if "image" in rel.target_ref:
                ...
                if rel.is_external:
                    ...
                    if response.status_code == 200:
                        ...
                        file_key = (
                            "image_files/" + self.tenant_id + "/" + file_uuid + "." + image_ext
                        )
                        mime_type, _ = mimetypes.guess_type(file_key)
                        await storage.save(file_key, response.content)
                    else:
                        continue
                else:
                    ...
                    await storage.save(file_key, rel.target_part.blob)
                # save file to db
                ...
                image_map[rel.target_part] = (
                    f"![image]({app_config.FILES_URL}/files/{upload_file.id}/file-preview)"
                )
    ...
    async def parse_docx(self, docx_path):
        doc = DocxDocument(docx_path)

        content = []

        image_map = await self._extract_images_from_docx(doc)
