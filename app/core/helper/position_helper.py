...
from utils.yaml_utils import load_yaml_file_cached
...
def get_position_map(folder_path: str, *, file_name: str = "_position.yaml") -> dict[str, int]:
    ...
    try:
        ...
        yaml_content = []
    positions = [
        item.strip() for item in yaml_content if item and isinstance(item, str) and item.strip()
    ]
...
def get_tool_position_map(folder_path: str, file_name: str = "_position.yaml") -> dict[str, int]:
    ...
    return pin_position_map(
        position_map,
        pin_list=app_config.POSITION_TOOL_PINS_LIST,
    )
