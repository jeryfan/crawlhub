...
const afterResponseErrorCode = (otherOptions: IOtherOptions): AfterResponseHook => {
  return async (_request, _options, response) => {
    ...
    if (!/^([23])\d{2}$/.test(String(clonedResponse.status))) {
      const bodyJson = clonedResponse.json() as Promise<ApiResponse>
      switch (clonedResponse.status) {
        case 403:
          bodyJson.then((data: ApiResponse) => {
            if (!otherOptions.silent)
              Toast.notify({ type: 'error', message: data.msg })
            if (data.code === ResponseCode.ALREADY_SETUP)
