 ...
 * the fastapiChatbotConfig should be defined in the html file before this script is included
 * the fastapiChatbotConfig should contain the token of the chatbot
...
(function () {
  ...
  const configKey = "fastapiChatbotConfig";
  const buttonId = "fastapi-chatbot-bubble-button";
  const iframeId = "fastapi-chatbot-bubble-window";
  ...
  const originalIframeStyleText = `
    ...
    top: unset;
    right: var(--${buttonId}-right, 1rem); /* Align with fastapi-chatbot-bubble-button. */
    bottom: var(--${buttonId}-bottom, 1rem); /* Align with fastapi-chatbot-bubble-button. */
    left: unset;
    width: 24rem;
  ...
  async function embedChatbot() {
    ...
    const baseUrl =
      config.baseUrl || `https://${config.isDev ? "dev." : ""}fastapi-template.com`;
    ...
    function createIframe() {
      ...
      iframe.title = "fastapi chatbot bubble window";
      iframe.id = iframeId;
    ...
    window.addEventListener('message', (event) => {
      ...
      if (event.data.type === 'fastapi-chatbot-iframe-ready') {
        targetIframe.contentWindow?.postMessage(
          {
            type: 'fastapi-chatbot-config',
            payload: {
      ...
      if (event.data.type === 'fastapi-chatbot-expand-change') {
        toggleExpand();
      }
    });
