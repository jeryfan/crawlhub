...
const AvatarEdit = ({
  ...
  const displayAvatar = previewUrl || avatar || null
  ...
  const { handleLocalFileUpload } = useLocalFileUploader({
    ...
    onUpload: (imageFile: ImageFile) => {
      if (imageFile.progress === 100) {
        ...
        setPreviewUrl(imageFile.url || null)
        onChange(imageFile.fileId)
        Toast.notify({ type: 'success', message: t('actionMsg.modifiedSuccessfully', { ns: 'common' }) })
      }
      if (imageFile.progress === -1) {
        setUploading(false)
        Toast.notify({ type: 'error', message: t('imageUploader.uploadFromComputerUploadError', { ns: 'common' }) })
      }
    },
  })
  ...
  const handleDeleteAvatar = useCallback(() => {
    ...
    setOnAvatarError(false)
    onChange(null)
  }, [onChange])

  if (DISABLE_UPLOAD_IMAGE_AS_ICON)
    return <Avatar name={name} avatar={displayAvatar} size={size} className={className} textClassName={textClassName} />
      ...
      <div className="inline-block">
        <div className="group relative inline-block" style={{ width: size, height: size }}>
          <Avatar
            ...
            onError={(x: boolean) => setOnAvatarError(x)}
          />
          <div
            className="absolute inset-0 flex cursor-pointer items-center justify-center rounded-full bg-black/50 opacity-0 transition-opacity group-hover:opacity-100"
            onClick={() => {
              if (hoverArea === 'right' && displayAvatar && !onAvatarError)
          ...
          >
            {hoverArea === 'right' && displayAvatar && !onAvatarError
              ? (
