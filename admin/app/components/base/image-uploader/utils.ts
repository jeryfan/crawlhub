 ...
 */
export const getImageUploadErrorMessage = (error: any, defaultMessage: string, t: (key: string, options?: Record<string, string>) => string): string => {
  const errorCode = error?.response?.code

  if (errorCode === ResponseCode.FORBIDDEN)
    return error?.response?.message

  if (errorCode === ResponseCode.FILE_EXTENSION_BLOCKED)
