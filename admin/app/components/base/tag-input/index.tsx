  ...
  return (
    <div
      className={cn(
        ...
        isSpecialMode
          ? 'rounded-lg bg-components-input-bg-normal pb-1 pl-1'
          : '',
      )}
    >
      {(items || []).map((item, index) => (
        <div
          key={item}
          className={cn(
            'system-xs-regular mr-1 mt-1 flex items-center rounded-md border border-divider-deep bg-components-badge-white-to-dark py-1 pl-1.5 pr-1 text-text-secondary',
          )}
        >
          {item}
          {!disableRemove && (
      ...
      {!disableAdd && (
        <div
          className={cn(
            ...
            !isSpecialMode
              ? 'rounded-md border border-dashed border-divider-deep px-1.5'
              : '',
          )}
        >
          {!isSpecialMode && !focused && (
            <RiAddLine className="h-3.5 w-3.5 text-text-placeholder group-hover/tag-add:text-text-secondary" />
          )}
          <AutosizeInput
            inputClassName={cn(
            ...
            placeholder={
              placeholder
                ? t(placeholder.replace(/^(common\.model\.params\.)/, 'model.params.').replace(/^(datasetDocuments\.segment\.)/, 'segment.'), { ns: placeholder.startsWith('common.') ? 'common' : 'datasetDocuments' })
                : (isSpecialMode
