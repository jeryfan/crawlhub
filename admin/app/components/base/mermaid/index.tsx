...
import LoadingAnim from '@/app/components/base/loading-anim'
...
const Flowchart = (props: FlowchartProps) => {
  ...
  const chartId = useRef(
    `mermaid-chart-${Math.random().toString(36).slice(2, 11)}`,
  ).current
  ...
  const handleRenderError = (error: any) => {
    ...
    try {
      ...
      console.error(
        'Failed to re-initialize Mermaid after error:',
        reinitError,
      )
    }
    ...
    setErrMsg(
      `Rendering failed: ${
        (error as Error).message || 'Unknown error. Please check the console.'
      }`,
    )
    setIsLoading(false)
  }
  ...
  const configureMermaid = useCallback(
    (primitiveCode: string) => {
      if (typeof window !== 'undefined' && isInitialized) {
        ...
        const isFlowchart
          = primitiveCode.trim().startsWith('graph')
            || primitiveCode.trim().startsWith('flowchart')
        ...
        `
          config.themeVariables = {
            ...
            primaryBorderColor:
              currentTheme === 'dark'
                ? THEMES.dark.connectionColor
                : THEMES.light.connectionColor,
          }
  ...
  const themeClasses = {
    ...
    themeToggle: cn(
      'flex h-10 w-10 items-center justify-center rounded-full shadow-md backdrop-blur-sm transition-all duration-300',
      {
        'bg-white/80 hover:bg-white hover:shadow-lg text-gray-700 border border-gray-200':
          currentTheme === Theme.light,
        'bg-slate-800/80 hover:bg-slate-700 hover:shadow-lg text-yellow-300 border border-slate-600':
          currentTheme === Theme.dark,
      },
    ),
  }
      ...
      {isLoading && !svgString && (
        <div className="px-[26px] py-4">
          ...
          <div className="mt-2 text-sm text-gray-500">
            {t(
              'wait_for_completion',
              { ns: 'common', defaultValue: 'Waiting for diagram code to complete...' },
            )}
          </div>
        </div>
      )}
