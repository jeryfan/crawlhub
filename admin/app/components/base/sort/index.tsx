...
const Sort: FC<Props> = ({ order, value, items, onSelect }) => {
  ...
  return (
    <div className="inline-flex items-center">
      ...
      <div
        className="ml-px cursor-pointer rounded-r-lg bg-components-button-tertiary-bg p-2 hover:bg-components-button-tertiary-bg-hover"
        onClick={() => onSelect(`${order ? '' : '-'}${value}`)}
      >
        {!order && (
          <RiSortAsc className="h-4 w-4 text-components-button-tertiary-text" />
        )}
        {order && (
          <RiSortDesc className="h-4 w-4 text-components-button-tertiary-text" />
        )}
      </div>
    </div>
  )
