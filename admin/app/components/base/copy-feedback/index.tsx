...
const CopyFeedback = ({ content }: Props) => {
  ...
  return (
    <Tooltip
      popupContent={
        (isCopied
          ? t('overview.appInfo.embedded.copied', { ns: 'app' })
          : t('overview.appInfo.embedded.copy', { ns: 'app' })) || ''
      }
    >
      <ActionButton>
...
export const CopyFeedbackNew = ({
  content,
  className,
}: Pick<Props, 'className' | 'content'>) => {
  const { t } = useTranslation()
  const [isCopied, setIsCopied] = useState<boolean>(false)
  ...
  return (
    ...
    >
      <div
        className={`h-8 w-8 cursor-pointer rounded-lg hover:bg-components-button-ghost-bg-hover ${
          className ?? ''
        }`}
      >
        <div
          ...
          className={`h-full w-full ${copyStyle.copyIcon} ${
