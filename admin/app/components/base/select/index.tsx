...
const Select: FC<ISelectProps> = ({
  ...
  useEffect(() => {
    ...
    defaultSelect
      = (defaultValue
        && items.find((item: Item) => item.value === defaultValue))
      || null
...
const SimpleSelect: FC<ISelectProps> = ({
  ...
  // Update selectedItem when value changes (Controlled mode)
  useEffect(() => {
    if (value !== undefined)
      setSelectedItem(items.find((item: Item) => item.value === value) ?? null)
  }, [value, items])
  ...
  useEffect(() => {
    ...
    if (!isCurrentSelectionValid) {
      ...
      // Handle cases where defaultValue might be undefined, null, or empty string
      defaultSelect
        = items.find((item: Item) => item.value === defaultValue) ?? null
      setSelectedItem(defaultSelect)
    }
      ...
      {({ open }) => (
        ...
        >
          ...
          {!disabled && (
            <ListboxOptions
              className={cn(
                'absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-xl border-[0.5px] border-components-panel-border bg-components-panel-bg-blur px-1 py-1 text-base shadow-lg backdrop-blur-sm focus:outline-none sm:text-sm',
                optionWrapClassName,
              )}
            >
              {items.map((item: Item) =>
  ...
  return (
    ...
    >
      ...
      <PortalToFollowElemContent className={`z-20 ${popupClassName}`}>
        <div
          className={cn(
            'max-h-60 overflow-auto rounded-xl border-[0.5px] border-components-panel-border bg-components-panel-bg px-1 py-1 text-base shadow-lg focus:outline-none sm:text-sm',
            popupInnerClassName,
          )}
        >
          {items.map((item: Item) => (
