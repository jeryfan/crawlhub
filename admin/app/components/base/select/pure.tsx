...
export type PureSelectProps = SharedPureSelectProps
...
const PureSelect = (props: PureSelectProps) => {
  ...
  const triggerText = useMemo(() => {
    ...
    if (multiple) {
      return value?.length
        ? t('dynamicSelect.selected', { ns: 'common', count: value.length })
        : placeholderText
    }

    return (
      options.find(option => option.value === value)?.label || placeholderText
    )
