...
const DatePicker = ({
  ...
  const [selectedMonth, setSelectedMonth] = useState(() =>
    (inputValue || defaultValue).month(),
  )
  const [selectedYear, setSelectedYear] = useState(() =>
    (inputValue || defaultValue).year(),
  )
  ...
  useEffect(() => {
    ...
    if (normalizedValue) {
      ...
      setSelectedDate(prev =>
        prev ? getDateWithTimezone({ date: prev, timezone }) : undefined,
      )
    }
  }, [timezone])
  ...
  const handleConfirmDate = useCallback(
    (passedInSelectedDate?: Dayjs) => {
      ...
      const nextDate = dayjs.isDayjs(passedInSelectedDate)
        ? passedInSelectedDate
        : selectedDate
  ...
  const handleDateSelect = useCallback(
    (day: Dayjs) => {
      const newDate = cloneTime(
        day,
        selectedDate || getDateWithTimezone({ timezone }),
      )
  ...
  const handleTimeSelect = (hour: string, minute: string, period: Period) => {
    const newTime = cloneTime(
      dayjs(),
      dayjs(`1/1/2000 ${hour}:${minute} ${period}`),
    )
    setSelectedDate((prev) => {
  ...
  const handleYearMonthConfirm = () => {
    setCurrentDate(prev =>
      prev.clone().month(selectedMonth).year(selectedYear),
    )
    setView(ViewType.date)
  }
            ...
            text-components-input-text-filled outline-none placeholder:text-components-input-text-placeholder"
                ...
                <RiCalendarLine
                  className={cn(
                    ...
                    (displayValue || (isOpen && selectedDate))
                    && 'group-hover:hidden',
                  )}
                />
                <RiCloseCircleFill
                  className={cn(
                    ...
                    (displayValue || (isOpen && selectedDate))
                    && 'hover:text-text-secondary group-hover:inline-block',
                  )}
                  onClick={handleClear}
                />
              </div>
