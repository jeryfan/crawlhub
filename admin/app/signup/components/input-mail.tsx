...
export default function Form({ onSuccess }: Props) {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    if ((res as MailSendResponse).token)
      onSuccess(email, (res as MailSendResponse).token)
        ...
        <>
          <div className="system-xs-regular mt-3 block w-full text-text-tertiary">
            ...
            <Link
              ...
              href="https://fastapi-template.com/terms"
            >
              {t('tos', { ns: 'login' })}
            </Link>
            &nbsp;&&nbsp;
            <Link
              ...
              target="_blank"
              rel="noopener noreferrer"
              href="https://fastapi-template.com/privacy"
            >
              {t('pp', { ns: 'login' })}
            </Link>
          </div>
