    ...
    <div className="bg-background-default-subtle">
      ...
      <div className={`mb-4 mt-5 flex flex-col gap-2 ${isLoggedIn ? 'pb-2' : ''}`}>
        <div className="title-4xl-semi-bold">
          ...
          <div className="text-[var(--color-saas-fastapi-blue-inverted)]">{authAppInfo?.app_label?.en_US || t('unknownApp', { ns: 'oauth' })}</div>
          {!isLoggedIn && <div className="text-text-primary">{t('tips.notLoggedIn', { ns: 'oauth' })}</div>}
        </div>
        <div className="body-md-regular text-text-secondary">{isLoggedIn ? `${authAppInfo?.app_label?.en_US || t('unknownApp', { ns: 'oauth' })} ${t('tips.loggedIn', { ns: 'oauth' })}` : t('tips.needLogin', { ns: 'oauth' })}</div>
      </div>
        ...
        {!isLoggedIn
          ...
          : (
              <>
                ...
                <Button size="large" className="w-full" onClick={() => router.push('/dashboard')}>{t('operation.cancel', { ns: 'common' })}</Button>
              </>
            )}
      </div>
