...
const EmailChangeModal = ({ onClose, email, show }: Props) => {
  ...
  const handleVerifyOriginEmail = async () => {
    await verifyEmailAddress(email, code, stepToken, () =>
      setStep(STEP.newEmail))
    setCode('')
  }
  ...
  const checkNewEmailExisted = async (email: string) => {
    ...
    try {
      ...
      if (e.status === 400) {
        const [, errRespData] = await asyncRunSafe<ApiResponse>(e.json())
        const { code } = errRespData || {}
        if (code === ResponseCode.EMAIL_ALREADY_IN_USE)
          setNewEmailExited(true)
        if (code === ResponseCode.ACCOUNT_IN_FREEZE)
  ...
  return (
    <Modal isShow={show} onClose={noop} className="!w-[420px] !p-6">
      <div
      ...
      {step === STEP.start && (
        <>
          <div className="title-2xl-semi-bold pb-3 text-text-primary">
          ...
          <div className="space-y-0.5 pb-2 pt-1">
            ...
            <div className="body-md-regular text-text-secondary">
              <Trans
                ...
                components={{
                  email: (
                    <span className="body-md-medium text-text-primary"></span>
                  ),
                }}
                values={{ email }}
              />
        ...
        <>
          ...
          <div className="system-xs-regular mt-3 flex items-center gap-1 text-text-tertiary">
            ...
            {!time && (
              <span
                onClick={sendCodeToOriginEmail}
                className="system-xs-medium cursor-pointer text-text-accent-secondary"
              >
      ...
      {step === STEP.newEmail && (
        <>
          ...
          <div className="pt-3">
            ...
            />
            {newEmailExited && (
              <div className="body-xs-regular mt-1 py-0.5 text-text-destructive">
