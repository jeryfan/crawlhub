...
export default function VerifyEmail(props: DeleteAccountProps) {
  ...
  const handleConfirm = useCallback(async () => {
    try {
      await confirmDeleteAccount({ code: verificationCode!, token: emailToken })
      props.onConfirm()
    }
  ...
  return (
    <>
      ...
      <div className="body-md-regular pb-2 pt-1 text-text-secondary">
        ...
        <Link href="https://fastapi-template.com/privacy" className="text-text-accent">{t('account.deletePrivacyLink', { ns: 'common' })}</Link>
      </div>
